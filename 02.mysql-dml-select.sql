...
SELECT CONCAT(first_name, ' ', last_name) AS 이름, salary AS 월급, phone_number AS 전화번호, hire_date AS 입사일 
	FROM employees ORDER BY hire_date, first_name;
    
-- 문제2.
SELECT job_title, max_salary, salary FROM employees, jobs ORDER BY salary DESC;
...
-- 문제6.
SELECT CONCAT(first_name, ' ', last_name) AS 이름, DATE_FORMAT(hire_date, '%Y-%m') AS 입사일, department_id AS 부서번호
	FROM employees WHERE department_id IN(10, 90, 100);
    
-- 문제7.
SELECT CONCAT(first_name, ' ', last_name) AS 이름, salary FROM employees 
...
-- 문제10.
SELECT CONCAT(first_name, ' ', last_name) AS 이름, salary, REPLACE(phone_number, ".", "-")As phone_number, hire_date FROM employees
WHERE hire_date < '2003-12-31';
-- 545.343.3433 -> 545-343-3433